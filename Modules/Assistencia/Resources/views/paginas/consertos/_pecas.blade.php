            ...
            @if(count($pecaOS) != 0)
                @foreach ($pecas as $item)
                    @foreach($pecaOS as $pec)
                        ...
                        <option data-valor="{{$item->peca->valor_venda}}" selected value="{{ $item->id }}">{{ $item->peca->nome }} |
                            {{ $item->peca->valor_venda }}</option>
                        @else
                        <option data-valor="{{$item->peca->valor_venda}}" value="{{ $item->id }}">{{ $item->peca->nome }} |
                            {{ $item->peca->valor_venda }}</option>
                        @endif
                    @endforeach
                @endforeach
