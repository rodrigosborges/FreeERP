...
<div class="container">
    <div class="card col-md-12">
        <div class="card-body"> 
            ...
            <form action="{{url('estoquemadeireira/vendas/cliente/busca')}}" method="POST" id="form">
            @csrf
            <div class="row">
                    <div class="form-group col-8">
                        <input type="text" id="search-input" maxlength="20" placeholder="Insira o nome do Cliente" class="form-control" name="pesquisa">
                    </div>   
            ...
            <table class="table text-center">
                <thead class="card-header">
                    <div class="col-12 text-right">
                        @if($flag == 0)
                            <a href="{{url('/estoquemadeireira/vendas/cliente/create')}}" class="btn btn-success btn-sm mt-2 mb-3">
                                <i class="material-icons" style="vertical-align:middle; font-size:25px;">note_add</i>Adicionar 
                            </a>
                            <a href="{{url('/estoquemadeireira/vendas/cliente/inativos')}}" class="btn btn-danger btn-sm mt-2 mb-3">
                    ...
                    <tbody>
                        ...
                        @foreach(clientes as cliente)
                            <tr>
                                <td>{{cliente->nome}}</td>
                                <td>{{cliente->quantidade_itens}}</td>
                                @if($flag == 0)
                                    <td>  
                                    <a href="{{url('/estoquemadeireira/tipounidade/' . cliente->id . '/edit')}}"><button class="btn btn-sm btn-warning"><i class="material-icons" style="font-size:18px;">border_color</i></button></a>
                                    </td>
                                    <td>
                                    <form method="POST" action="{{url('/estoquemadeireira/tipounidade/' . cliente->id)}}">
                                    @method('delete')
                                    @csrf
                                ...
                                @else
                                    <td>
                                        <form method="POST" action="{{url('/estoquemadeireira/tipounidade/' . cliente->id. '/restore')}}">
                                        @method('put')
                                        @csrf
