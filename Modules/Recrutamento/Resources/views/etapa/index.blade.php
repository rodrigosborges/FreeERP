...
@section('content')
    ...
    <div class="row">
        ...
        <form action="{{url('recrutamento/etapa/')}}" method="GET" class="form-inline mb-2">
            ...
            <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    <a class="nav-item nav-link active" id="nav-ativos-tab" data-toggle="tab" href="#nav-ativos" role="tab" aria-controls="nav-ativos" aria-selected="true">Ativas ({{$data['etapas']->count()}})</a>
                    <a class="nav-item nav-link" id="nav-inativos-tab" data-toggle="tab" href="#nav-inativos" role="tab" aria-controls="nav-inativos" aria-selected="false">Inativas ({{$data['etapas_inativas']->count()}})</a>
                </div>
        ...
        <div class="col-sm-12 col-md-2 offset-md-4">
            <a class="btn btn-success " style="margin-bottom:10px;" href="{{ url('recrutamento/etapa/create') }}"><i class="material-icons" style=" vertical-align: middle;">add_circle_outline</i>Nova Etapa</a>
        </div>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="{{url('recrutamento/etapa', [$item->id])}}" method="POST">
                                {{method_field('DELETE')}}
                                {{ csrf_field() }}
                                <a class="btn btn-warning" href='{{ url("recrutamento/etapa/$item->id/edit") }}'><i class="material-icons" style=" vertical-align: middle;">edit</i>Editar</a> 
            ...
            <table class="table table-striped">
                ...
                @foreach($data['etapas_inativas'] as $item)
                    <tr>
                        <td>{{$item->nome}}</td>
                        <td>
                            <a class="btn btn-info" href='{{ url("recrutamento/etapa/$item->id/restore") }}'><i class="material-icons" style=" vertical-align: middle;">restore_from_trash</i>Restaurar</a> 
                        </td>
                    </tr>
                @endforeach
                </tbody>
